...
class MumoGaussianMLPPolicy(Policy):
    """
    ...
    def __init__(
        self, *args, meta_batch_size, rollouts_per_meta_task,
        max_path_length, init_std=1., min_std=1e-6, use_betas=False,
        shift_gammas=False, mlp_after_gru=True, mlp_after_pool=True,
        **kwargs):
        ...
        self.cell_size = kwargs.get('cell_size', 32)
        if use_betas:
            self.modulation_vector_size = [size * 2 for size in self.hidden_sizes]
        else:
            self.modulation_vector_size = [size for size in self.hidden_sizes]
        self.use_betas = use_betas
        self.shift_gammas = shift_gammas
        self.meta_batch_size = meta_batch_size
        self.rollouts_per_meta_task = rollouts_per_meta_task
        self.max_path_length = max_path_length

        self.mlp_after_gru = mlp_after_gru
        self.mlp_after_pool = mlp_after_pool
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('task_network'):
            task_net_input_shape = (
                self.meta_batch_size,
                None,
                self.max_path_length,
                self.obs_dim + self.action_dim + 1,
            )
            (self.mod_input_var, self.num_paths_var, self.mod_var,
             self.task_embedding) = create_task_net(
                name='modulation_network',
                modulation_vector_size=self.modulation_vector_size,
                hidden_nonlinearity=self.hidden_nonlinearity,
                output_nonlinearity=self.output_nonlinearity,
                cell_size=self.cell_size,
                input_dim=task_net_input_shape,
                mlp_after_gru=self.mlp_after_gru,
                mlp_after_pool=self.mlp_after_pool,
            )
            current_scope = tf.get_default_graph().get_name_scope()
            task_vars = tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, scope=current_scope)
            self.all_policy_params = OrderedDict(
                [(remove_scope_from_name(
                    var.name, current_scope), var) for var in task_vars])
        with tf.variable_scope(self.name):
            # build the actual policy network
            self.obs_var, self.mean_var = create_mlp(
                name='mean_network', output_dim=self.action_dim,
            ...
            with tf.variable_scope("log_std_network"):
                log_std_var = tf.get_variable(
                    name='log_std_var', shape=(1, self.action_dim,),
                    dtype=tf.float32, initializer=tf.constant_initializer(
                        self.init_log_std),
                    trainable=self.learn_std)

                self.log_std_var = tf.maximum(
                    log_std_var, self.min_log_std, name='log_std')
        ...
        self.all_policy_params.update(self.policy_params)

    def get_params(self):
        return self.all_policy_params
    ...
    def get_action(self, observation):
        ...
        action, agent_infos = action[0], dict(
            mean=agent_infos['mean'][0],
            log_std=agent_infos['log_std'][0])
    ...
    def get_actions(self, observations):
        ...
        actions, means, logs_stds = sess.run(
            [self.action_var, self.mean_var, self.log_std_var],
    ...
    def distribution_info_sym(self, mod_var, obs_var, params=None):
        """
        Return the symbolic distribution information about the actions.

        Args:
            modulation_var (placeholder) : symbolic variable for task network outputs
        ...
        if params is None:
            with tf.variable_scope(self.name):
                obs_var, mean_var = create_mlp(
                    ...
                    hidden_nonlinearity=self.hidden_nonlinearity,
                    output_nonlinearity=self.output_nonlinearity,
                    input_var=obs_var, reuse=True)
                log_std_var = self.log_std_var
        elif mod_var is None:
            ...
            assert len(log_std_network_params) == 1
            obs_var, mean_var = forward_modulated_mlp(
                output_dim=self.action_dim, hidden_sizes=self.hidden_sizes,
                hidden_nonlinearity=self.hidden_nonlinearity,
                output_nonlinearity=self.output_nonlinearity,
                input_var=obs_var,
                mod_vars=mod_var,
