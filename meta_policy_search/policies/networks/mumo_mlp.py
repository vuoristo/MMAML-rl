...
def create_task_net(name,
    ...
    input_var = tf.placeholder(
            dtype=tf.float32, shape=input_dim, name='input')

    num_paths_var = tf.placeholder(
        dtype=tf.float32, shape=(input_dim[0],), name='num_paths')

    input_dim = list(map(lambda x: x if x is not None else -1, input_dim))

    # flatten the rollout dimension
    x = tf.reshape(input_var, (-1, *input_dim[2:]))

    # Setup the RNN cell
    x = tf.keras.layers.GRU(cell_size)(x)

    if mlp_after_gru:
        x = tf.keras.layers.Dense(
            cell_size, activation='relu')(x)
        x = tf.keras.layers.Dense(cell_size)(x)

    # unflatten rollout dimension
    x = tf.reshape(x, (*input_dim[:2], cell_size))
    x = tf.reduce_sum(x, axis=1) / tf.reshape(num_paths_var, (-1, 1))
    ...
    output_dim = sum(modulation_vector_size)
    modulation_vector = tf.keras.layers.Dense(
        output_dim,
        kernel_initializer=w_init,
        bias_initializer=b_init,
    )(x)

    return input_var, num_paths_var, modulation_vector, task_embedding
...
def modulate(x, mod_vars, size, use_betas=False, shift_gammas=False):
    if use_betas:
        gammas, betas = tf.split(mod_vars, (size, size), axis=-1)
    else:
        gammas = mod_vars
    if shift_gammas:
        gammas = gammas + tf.ones_like(gammas)
...
def forward_modulated_mlp(
    ...
    mod_vars = tf.split(mod_vars, [h for h in mod_size], axis=-1)
    ...
    for name, param in mlp_params.items():
        ...
        if bias_added:
            if "hidden" in name:
                x = modulate(x, mod_vars[idx], sizes[idx], use_betas,
                             shift_gammas)
                x = hidden_nonlinearity(x)
            elif "output" in name:
                x = output_nonlinearity(x)
