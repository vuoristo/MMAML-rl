...
def flat_to_padded_sequences(
    flat_sequence, sequence_lengths, max_rollouts, max_path_length):
    element_shape = flat_sequence.shape[1:]
    padded_seq = np.zeros((max_rollouts, max_path_length, *element_shape))
    prev_index = 0
    for i, length in enumerate(sequence_lengths):
        cur_index = prev_index + length
        (padded_seq[i, max_path_length - length:]
            ) = flat_sequence[prev_index:cur_index]
