...
class MAMLFirstOrderOptimizer(Optimizer):
    ...
    def build_graph(self, loss, target, input_ph_dict):
        ...
        self._loss = loss
        grads_and_var = self._tf_optimizer.compute_gradients(loss, target.get_params())
        self._grad_norms = {v.name: tf.norm(g) for g, v in grads_and_var}
    ...
    def optimize(self, input_val_dict, extra_feed_dict={}):
        """
        ...
        for epoch in range(self._max_epochs):
            ...
            loss, _, grad_norms = sess.run([
                self._loss, self._train_op, self._grad_norms], feed_dict)
            ...
            #     break
            # last_loss = new_loss
        return loss_before_opt, grad_norms
...
class MAMLPPOOptimizer(MAMLFirstOrderOptimizer):
    ...
    def compute_stats(self, input_val_dict, extra_feed_dict={}):
        """
